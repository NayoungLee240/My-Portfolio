...
export const swaggerSpec = {
  ...
  components: {
    securitySchemes: {
      BearerAuth: { type: 'http', scheme: 'bearer', bearerFormat: 'JWT', description: 'Access Token (매 API 호출마다 재발급)' },
    },
    schemas: {
      LoginRequest: {
  ...
  tags: [
    { name: 'Auth', description: 'JWT 기반 인증 API — 요청 단위 토큰 재발급, Redis 세션 검증, 중복 로그인 방지' },
  ...
  paths: {
    ...
    '/admin/metrics/ccu': { get: { tags: ['Admin (운영툴)'], summary: 'CCU 추이 (실시간 집계)', description: 'MySQL 기반 실시간 쿼리로 집계한 동접자 추이 데이터를 반환합니다.', security: [{ BearerAuth: [] }], parameters: [{ name: 'from', in: 'query', schema: { type: 'string', format: 'date-time' } }, { name: 'interval', in: 'query', schema: { type: 'string', enum: ['5m', '30m', '1h', '1d'], default: '5m' } }], responses: { 200: { description: 'CCU 시계열 데이터', content: { 'application/json': { schema: { $ref: '#/components/schemas/CcuMetrics' } } } } } } },
  },
};
